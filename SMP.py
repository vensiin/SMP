...
pt_values, at_values = [], [] # Practice test values and actual test values

# Sliding window
for i in range(len(test_data) - num_unrollings):
    pt_values.append(test_data[i:i + num_unrollings]) # 50 real prices
    at_values.append(test_data[i + num_unrollings]) # The next price after the 50 real prices

# print(f"sliding window for pt: {pt_values}")
# print(f"sliding window for at: {at_values}")
...
# Non-data augmentation predictions/ test predictions
test_predictions = model.predict(pt_values, verbose=0).flatten()
# MSE for non-data augmentation predictions
mse_tp = np.mean((at_values - test_predictions) ** 2)
naive_test_pred = pt_values[:, -1, 0]
naive_test_mse = np.mean((at_values - naive_test_pred) ** 2)

print(f"Scaled Model MSE: {model_mse:.6f}") # Model MSE
print(f"Scaled Naive MSE: {naive_mse:.6f}") # Naive MSE

# Reshapes the predictions to add as a parameter to unscale
test_predictions_copy = test_predictions.copy().reshape(-1,1) # Creates a copy of test predictions and reshapes for unscaling (2d array)
# Unscaled test predictions
us_test_predictions = Scaler.inverse_transform(test_predictions_copy).reshape(-1) # Reshapes back into 1d array
print(f"Unscaled test predictions: {us_test_predictions}")

# Unscaled test data
actual_values_copy = test_data.copy().reshape(-1,1)
us_actual_td = Scaler.inverse_transform(actual_values_copy).reshape(-1)
print(f"Actual values: {us_actual_td}")

# Unscaled test predictions & unscaled actual values
us_tp, us_av = [], []
# Sliding window
for i in range(len(test_data) - num_unrollings):
    us_tp.append(actual_values_copy[i:i + num_unrollings]) # 50 real prices
    us_av.append(actual_values_copy[i + num_unrollings]) # The next price after the 50 real prices

# Unscaled test predictions
us_tp = np.array(us_tp).reshape(-1, num_unrollings, 1) # Reshapes to enter model
us_av = np.array(us_av) # Transforms into a numpy array
unscaled_predictions = model.predict(us_tp, verbose=0).flatten() # Returns a numpy array of predictions

mse_us_tp = np.mean((us_av - unscaled_predictions) ** 2) # Calculate the MSE
print(f"Unscaled MSE: {mse_us_tp:.5f}")

naive_us_td = us_tp[:, -1, 0]
naive_us_test_mse = np.mean((us_av - naive_us_td) ** 2)
print(f"Unscaled Naive MSE: {naive_us_test_mse:.5f}")

fig, ax = plt.subplots(figsize=(15, 10))

ax.plot(unscaled_predictions[:100], label='Predictions', marker='o', linewidth=2, markersize=4, alpha=0.7) # Plots the first 100 actual values. The x-axis is generated by matplot
ax.plot(us_av[:100], label='Actual', linewidth=2, markersize=4, alpha=0.7)
ax.set_xlabel('Sample Index', fontsize=12)
ax.set_ylabel('Normalized Price', fontsize=12)
ax.set_title('Unscaled LSTM Predictions vs Actual (First 100 Samples) lambda method', fontsize=14, fontweight='bold')
